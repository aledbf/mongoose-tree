        ...
        if(isParentChange) {
          ...
          self.collection.find({ path : { '$regex' : '^' + previousPath + '.' } }, function(err, cursor) {
            ...
            stream.on('data', function (doc) {
              ...
              self.collection.update({ _id : doc._id }, { $set : { path : newPath, latest: true } }, function(err) {
                if(err) return next(err);
                // Remove latest attribute from parent (if exists)
                self.collection.update({ _id : self.parent }, { $unset: { 'latest' : 1 } }, function(err) {
                  if(err) return next(err);
                });
              });
            });
